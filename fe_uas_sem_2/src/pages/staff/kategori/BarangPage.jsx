...
const BarangPage = () => {
  ...
  const columns = [
    { header: 'ID', key: 'kategori_barang_id', width: '10%' },
    { header: 'Kategori Barang', key: 'nama_kategori', width: '70%' },
    { 
      header: 'Aksi', 
      key: 'aksi', 
      width: '20%',
      render: (row) => (
        <div className="flex gap-3 justify-center">
          <button 
            ...
            className="p-2 bg-[#2D3E5E] text-white rounded-md hover:scale-110 transition-transform"
          >
            <FiEdit size={18} />
          </button>
          <button 
            onClick={() => handleDelete(row.kategori_barang_id)}
            className="p-2 bg-[#C64747] text-white rounded-md hover:scale-110 transition-transform"
          >
            <FiTrash2 size={18} />
          </button>
        </div>
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 w-full min-h-screen bg-white font-['Montserrat']">
      {/* Header Judul */}
      <h1 className="text-2xl md:text-3xl font-bold text-[#2D3E5E] mb-6 md:mb-8 text-center md:text-left">Kelola Kategori</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-md p-8 rounded-3xl shadow-2xl relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {isLoading ? 'Menyimpan...' : (editId ? 'Simpan Perubahan' : 'Tambah Kategori')}
              </button>
            </form>
          </div>
        </div>
