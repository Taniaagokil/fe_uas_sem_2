...
const GedungPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        ...
        showToast('Gedung berhasil ditambahkan', 'success');
      }
  ...
  const handleDelete = (id) => {
    showConfirm({
      title: 'Hapus Gedung',
      message: 'Apakah Anda yakin ingin menghapus gedung ini?',
      onConfirm: async () => {
        try {
          ...
          showToast(err.response?.data?.message || 'Gagal menghapus gedung', 'error');
        }
      }
    });
  };
  ...
  const columns = [
    { 
      header: 'ID', 
      width: '15%',
      render: (row) => <span className="text-[#273A5A] font-bold">#{row.gedung_id}</span>
    },
    { 
      header: 'Nama Gedung', 
      align: 'left',
      width: '60%',
      render: (row) => <span className="font-extrabold text-[#273A5A] text-xs md:text-sm uppercase tracking-wider bg-[#273A5A]/5 px-3.5 py-2 rounded-full">{row.nama_gedung}</span>
    },
    { 
      header: 'Aksi', 
      align: 'center',
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 w-full">
          <div className="space-y-1">
            <h1 className="text-2xl md:text-3xl font-extrabold text-[#273A5A] tracking-tight">
              Kelola <span className="text-[#E2B053]">Gedung</span>
            </h1>
            <p className="text-slate-400 text-xs md:text-sm font-medium">Tambah, edit, dan hapus gedung/lokasi kampus</p>
          </div>
        </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 ml-1">Nama Gedung</label>
                  <input 
                    type="text"
                    required
                    value={namaGedung}
